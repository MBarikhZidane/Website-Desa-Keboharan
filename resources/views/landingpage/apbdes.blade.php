<x-landingpage.app :setting="$setting">
    <div class="bg-gray-50 text-gray-800 pt-20">
        ...
        <section class="py-10 px-6 text-center">
            ...
            <div class="mb-10">
                ...
                <div class="relative inline-block">
                    <select id="tahunSelect"
                        class="appearance-none border border-gray-300 rounded-xl px-4 py-2 pr-10 bg-white text-gray-700 font-medium shadow-sm focus:outline-none focus:ring-2 focus:ring-main/60 focus:border-main transition-all duration-200 ease-in-out hover:border-main/70">
                    ...
                    <!-- Ikon panah -->
                    <svg class="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-main pointer-events-none"
            ...
            <!-- Kartu Statistik -->
            <div class="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6" id="statCards">
        ...
        <section class="py-10 px-6 bg-white">
            ...
            <div class="max-w-4xl md:mx-auto mx-0">
                <canvas id="chartBelanja"></canvas>
                                <div class="overflow-x-auto mt-10">
                    <table class="w-full border border-gray-300 text-sm" id="tabelBelanja">
                        <thead class="">
                            <tr>
                                <th class="p-3 border">Nama Kegiatan</th>
                                <th class="p-3 border">Total Anggaran</th>
        ...
        <section class="py-10 px-6 hidden">
            <div class="max-w-4xl mx-auto">
    ...
    `;
            ...
            updateCharts(tahun);
            updateTabelPendapatan(tahun);
            updateTabelBelanja(tahun);
        }
        ...
        const chart1 = new Chart(document.getElementById("chartPendapatanBelanja"), {
            ...
            data: {
                ...
                datasets: [{
                        label: "Pendapatan",
                        backgroundColor: "#1D4700",
                        data: Object.values(dataAPBDes).map(d => d.pendapatan)
                    },
                    {
                        label: "Belanja",
                        backgroundColor: "#F87171",
                        data: Object.values(dataAPBDes).map(d => d.belanja)
                    },
                ]
            },
            options: {
        ...
        function updateTabelBelanja(tahun) {
            const tbody = document.querySelector("#tabelBelanja tbody");
            tbody.innerHTML = "";

            const rows = dataAPBDes[tahun].rincianbelanja;

            rows.forEach(item => {
                tbody.innerHTML += `
                <tr class="hover:bg-gray-50">
                    <td class="p-3 border">${item.name}</td>
                    <td class="p-3 border">Rp${Number(item.total_anggaran).toLocaleString("id-ID")}</td>
                    <td class="p-3 border">Rp${Number(item.realisasi ?? 0).toLocaleString("id-ID")}</td>
                    <td class="p-3 border">Rp${Number(item.sisa ?? 0).toLocaleString("id-ID")}</td>
                </tr>
            `;
            });
        }

        function updateTabelPendapatan(tahun) {
            const tbody = document.querySelector("#tabelPendapatan tbody");
            tbody.innerHTML = "";

            const rows = dataAPBDes[tahun].rincianpendapatan;
