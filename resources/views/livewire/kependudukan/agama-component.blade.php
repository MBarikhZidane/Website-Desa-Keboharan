    ...
    <main class="p-6">
        <div class="bg-white p-6 rounded-lg shadow">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-lg font-bold">Daftar Pemeluk Agama</h2>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-200">
                            <th class="p-3">No</th>
                            <th class="p-3">Nama Agama</th>
                            <th class="p-3">Jumlah Pemeluk</th>
                    ...
                    <tbody>
                        @forelse($pemeluks as $index => $pemeluk)
                            <tr class="border-b border-gray-200-700">
                                ...
                                <td class="p-3">{{ $pemeluk->agama->name }}</td>
                                <td class="p-3">{{ $pemeluk->jumlah }}</td>
                                <td class="p-3 space-x-2">
                                    <button wire:click="edit({{ $pemeluk->id }})"
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div class="bg-white rounded-lg shadow-lg w-full max-w-lg p-6">
                <h3 class="text-lg font-semibold mb-4">
                    {{ $isEdit ? 'Edit Pemeluk agama' : 'Tambah Pemeluk agama' }}
                </h3>

                <form wire:submit.prevent="{{ $isEdit ? 'update' : 'store' }}">
                    <div class="mb-3">
                        <label class="block text-sm font-medium text-gray-700">Agama</label>
                        <select wire:model="agama_id"
                    ...
                    <div class="mb-3">
                        <label class="block text-sm font-medium text-gray-700">Jumlah Pemeluk</label>
